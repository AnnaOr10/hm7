...
namespace hm7
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < TrajectoryNumber; i++)
            {
                ...
                for (int X = 0; X < numberoftrial; X++)
                {
                    ...
                    int xCord = FromXrealToXVirtual(X, minX, maxX, rect.Width);
                    int yCord = FromYRealToYVirtual(Yt, minY, maxY, rect.Height);
        ...
        public void interarrivalHistogram(Dictionary<int, int> interarrivalDist, int numberoftrial)
        {
            ...
            g2.DrawRectangle(new Pen(Color.IndianRed), 0, 0, pictureBox3.Width - 1, pictureBox3.Height - 1);

            

            foreach (var item in interarrivalDist)
            {
                double virtualX = FromXrealToXVirtual(item.Value, 0, numberoftrial, pictureBox3.Height);
                g2.DrawRectangle(new Pen(Color.Black), j + 1, pictureBox3.Height - (int)virtualX - 1, step, (int)virtualX);
                g2.FillRectangle(Brushes.Blue, j + 1, pictureBox3.Height - (int)virtualX - 1, step, (int)virtualX);
                j += step;
            }
        ...
        public int FromYRealToYVirtual(double Y, double minY, double maxY, int H)
        {
